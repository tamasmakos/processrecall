"""GraphKnowsStore — a langgraph ``BaseStore`` backed by processrecall memory.

Maps langgraph's store protocol onto the processrecall verbs:
...
This module requires the ``langgraph`` extra (``pip install processrecall[langgraph]``);
...
def _session_id(namespace: tuple[str, ...]) -> str:
    """Flatten a langgraph namespace tuple into a processrecall session id."""
...
class GraphKnowsStore(BaseStore):  # type: ignore[misc]
    """A langgraph ``BaseStore`` whose long-term memory is a processrecall backend.

    Args:
        memory: A processrecall :class:`~processrecall.Memory` (or any object exposing
    ...
    async def abatch(self, ops: Iterable[Op]) -> list[Result]:
        """Async dispatch of langgraph store operations onto processrecall verbs."""
        results: list[Result] = []
        for op in ops:
    ...
    async def _put(self, op: PutOp) -> None:
        if op.value is None:
            # A None value is a delete in langgraph; processrecall purges by session.
            return None
    ...
    async def _search(self, op: SearchOp) -> list[SearchItem]:
        ...
        for i, hit in enumerate(hits):
            ...
            # resolved SearchItem to Any and mypy stayed silent. processrecall does
