...
``python -m processrecall.integrations.claude_code.hooks <verb>`` reads a hook
...
:mod:`processrecall.integrations.client`, so the hook process loads no
...
def _state_path() -> Path:
    ...
    default = Path.home() / ".processrecall" / "hook-checkpoints.json"
...
async def _run_against_service(name: str, event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    command = os.environ.get("GRAPHKNOWS_MCP_COMMAND", "processrecall-mcp")
