"""Async JSON-RPC stdio client for the processrecall MCP server.

Stdlib-only (asyncio + json) so it stays in the core install: any out-of-process
consumer can drive ``processrecall-mcp`` without pulling the ML/extraction extras.
"""
...
class GraphKnowsMCPClient:
    """Async JSON-RPC client for ``processrecall-mcp`` over stdio."""

    command: Sequence[str] | str = "processrecall-mcp"
        ...
        if self.initialize:
            await self.request(
                "initialize",
                {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "processrecall-client", "version": "1.0.0"},
                },
            )
