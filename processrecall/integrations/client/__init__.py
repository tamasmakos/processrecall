"""processrecall.integrations.client — official out-of-process client for the MCP server.

Stdlib-only (no ML/extraction dependencies), so it installs with the core
package. Drive ``processrecall-mcp`` from any framework::
...
``RemoteMemory`` wrapper mirroring :class:`processrecall.Memory`'s verbs used to sit
...
from processrecall.integrations.client.mcp import (
    GraphKnowsMCPClient,
    MCPClientError,
