...
:class:`~processrecall.storage.arcadedb.graph_store.GraphStore`. Nothing writes a
...
def __getattr__(name: str) -> Any:
    ...
    Keeps the query stack out of ``import processrecall.retrieval``.
    """
    lazy = {
        "DETRetriever": ("processrecall.retrieval.retriever", "DETRetriever"),
    }
    target = lazy.get(name)
    if target is None:
        raise AttributeError(f"module 'processrecall.retrieval' has no attribute {name!r}")
    import importlib
