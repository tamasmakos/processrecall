...
:func:`~processrecall.symbolic.ontology.rdf_io.parse_file`, which is what keeps this
...
log = logging.getLogger("processrecall.symbolic.ontology.loader")
...
def _digest_payload(path: Path) -> dict[str, Any] | None:
    ...
    A digest is our own pre-parsed shape (``python -m processrecall.symbolic.ontology.digest``):
...
def _terms_from_summary_json(path: Path) -> list[OntologyTerm]:
    ...
    for kind, key in (("class", "classes"), ("property", "properties")):
        for label in data.get(key) or []:
            ...
            terms.append(
                OntologyTerm(
                    uri=f"urn:processrecall:{stem}:{kind}:{label}",
                    label=label,
