...
The load-bearing pair is :func:`~processrecall.symbolic.ontology.loader.load_ontology_terms`
(classes and properties *with their definitions*, from a file or a folder) and
:func:`~processrecall.symbolic.ontology.catalog.load_ontology_index` (those definitions
...
Bring your own ontology with ``python -m processrecall.symbolic.ontology.digest`` (see
:mod:`processrecall.symbolic.ontology.digest`), then point ``GRAPHKNOWS_ONTOLOGY`` at the
result.

These imports stay LLM-free. The LLM decoder lives in
``processrecall.ingestion.extraction.llm``.
...
from processrecall.symbolic.ontology.labels import COARSE, entity_labels, relation_labels
from processrecall.symbolic.ontology.lexical import evoked, usable_index
from processrecall.symbolic.ontology.loader import (
    OntologyTerm,
    RdfOntologyManifest,
    load_ontology_terms,
