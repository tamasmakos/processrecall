...
Plugin files at repo root: `.claude-plugin/plugin.json`, `hooks/hooks.json`, `.mcp.json`,
`skills/`, alongside `pyproject.toml`. SessionStart bootstrap runs `uv sync` into a venv
under `CLAUDE_PLUGIN_DATA` once; all hooks and the MCP server invoke that venv's python by
absolute path. Clear error if uv is missing. Extras opted into via a plugin setting.
Development via `--plugin-dir`; marketplace later.
