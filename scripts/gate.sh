...
if [ "$CONTAINER_HAS_THIS_TREE" -eq 0 ]; then
  # MSYS_NO_PATHCONV for the same reason as the `docker run` below: under Git
  # Bash the runtime rewrites `/tmp/ruffcache` to `C:/Users/.../Temp/ruffcache`
  # before docker sees it, and that spelling is *relative* inside the container,
  # so ruff writes its cache to `/app/C:/Users/...` — a `C:` directory in the
  # worktree, regenerated by every gate run.
  dex() {
    MSYS_NO_PATHCONV=1 docker exec -e RUFF_CACHE_DIR=/tmp/ruffcache "$CONTAINER" \
      sh -c "cd /app && $*"
  }
else
