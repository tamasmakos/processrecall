...
async def test_memory_purge_delegates_to_runtime() -> None:
    ...
    from processrecall.server.mcp.tools.admin import memory_purge
    ...
    with patch(
        "processrecall.server.mcp.tools.admin.get_runtime", new=AsyncMock(return_value=runtime)
    ) as get_runtime:
...
async def test_memory_drop_namespace_delegates() -> None:
    ...
    from processrecall.server.mcp.tools.admin import memory_drop_namespace
