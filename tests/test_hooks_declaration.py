...
def declaration() -> dict[str, Any]:
    ...
    By convention and not through the manifest: the harness loads
    `hooks/hooks.json` from this fixed path on its own, and a manifest that
    names it as well is a duplicate the loader refuses — taking the whole
    plugin down with it, hooks and tool server alike.
    """
    path = REPO_ROOT / "hooks" / "hooks.json"
    assert path.is_file(), f"{path} is the conventional hooks path but is not shipped"
...
def test_every_verb_command_guards_on_the_prepared_venv_marker(
    declaration: dict[str, Any],
) -> None:
    """R14: when bootstrap could not build the venv the interpreter does not
    exist, and an unguarded command would fail every single turn with 127 —
    "nothing here fails loudly against a developer's own turn". Bootstrap
    already reported that failure once, at session start; these six channels
    must stay silent, so each tests the `.ready` marker bootstrap writes on a
    successful sync before it reaches for the interpreter."""
    marker = '[ -e "$CLAUDE_PLUGIN_DATA/venv/.ready" ]'
    for event in CONTRACT:
        if event in SCRIPTED_EVENTS:
            continue
        for command in commands(declaration, event):
            shell = command["command"]
            assert marker in shell, f"{event} invokes the interpreter unguarded: {shell}"
            assert shell.index(marker) < shell.index(INTERPRETER), (
                f"{event} tests the marker only after the interpreter: {shell}"
            )
